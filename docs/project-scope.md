...
- Manager/Admin dashboard now offers a direct create-ticket shortcut, and the job-ticket create/edit form supports focused quick-add for job type reference values, service locations, and equipment. Service-location and equipment quick-add use existing master-data APIs and keep quick-added equipment tied to the selected customer and service location context.
...
- the Manager/Admin dashboard now links directly to ticket creation, and the Manager/Admin job-ticket create/edit form can quick-add a job type value, service location, or equipment record from the ticket workflow, then select the newly added value back into the form;
