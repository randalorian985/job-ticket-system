...
- Manager/Admin dashboard now offers a direct create-ticket shortcut, and the job-ticket create/edit form supports focused quick-add for job type reference values, customers, service locations, and equipment. Customer quick-add uses the existing customer master-data API, selects the new customer as both ticket customer and billing party, carries customer contact details into billing-contact fields, and clears dependent service-location/equipment context before save. Service-location and equipment quick-add use existing master-data APIs, keep quick-added equipment tied to the selected customer and service location context, warn about possible duplicate equipment from loaded records before save, and show recent selected-equipment service-history context from existing report data.
...
- Employee service-ticket parts workflow now treats technicians as field-note users: they enter the part name/description, quantity, notes, and optional office-order request context on the ticket, while back office owns part numbers, catalog matching, cost, billable price, billing review, and invoice-readiness decisions.
...
- Employee mobile workflow for assigned jobs, GPS time tracking, work notes, part-name/description parts-used entry, files/photos, active-status-aware assigned-job list/detail field-context review, next field-context fix guidance, and pre-work context guidance.
- Employee parts-on-ticket entry for field technicians, including part name/description, quantity, notes, and optional office-order request flagging without exposing part number, pricing, billing, or catalog/master-data controls to employees.
- Manager/Admin job-ticket workflow, assignment management, archive/status UX, reporting hub, inline status/archive review feedback, closeout/invoice-readiness review, dashboard dispatch-readiness summary, dashboard create-ticket shortcut, dispatch-readiness list cues and filters, active-status-aware detail dispatch-readiness checklist and next-fix cues, edit-side dispatch-readiness cues, focused create/edit quick-add for job type, customer, service location, and equipment context with duplicate-equipment warnings, read-only selected-equipment service-history context from existing report data, and time-review approval workspace.
...
- the Employee job-ticket detail page lets technicians record work performed and parts used as field-ticket content, without asking employees for part numbers, costs, billable pricing, or billing decisions.
...
Technician parts-on-ticket entry is allowed as a job-ticket workflow improvement when it stays field-focused: work performed and parts used. Technicians should enter part name/description, quantity, notes, and optional office-order context only. Part numbers, catalog matching, costs, billable prices, billing review, and customer-facing financial decisions stay with Manager/Admin back-office workflows. This does not approve purchase orders, receiving, vendor invoicing, landed cost, truck inventory, warehouse transfers, replenishment, recommendations, AI/scoring, or automatic compatibility decisions.

Job-ticket form quick-add is allowed as a Manager/Admin creation/editing convenience when it stays tied to the current ticket context. Customer quick-add may create a customer through the existing customer master-data API, select that customer for the ticket and billing party, copy customer contact details into billing-contact fields, and clear dependent location/equipment context so the user adds the proper service location next. Service-location and equipment quick-add must preserve the selected customer and service-location relationship, must use DTO APIs, and may warn about possible duplicate equipment from loaded selected-context records without blocking the existing save path. It must not expand into broad master-data management, purchasing, receiving, inventory expansion, recommendations, accounting, payment, or invoice-generation behavior.

Manager/Admin job-ticket creation follow-ons are allowed as future scoped slices when they improve ticket creation quality without turning into a broad ERP or master-data expansion. Remaining approved candidate slices include create-from-recent-ticket, draft tickets, ticket templates, attachments during ticket creation, audit trail visibility, and role-specific guardrails. Any slice that needs API or schema changes must stay narrow, DTO-based, authorization-preserving, and documented before implementation.
