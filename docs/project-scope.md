...
- Manager/Admin dashboard and job ticket list dispatch-readiness cues now summarize active tickets that are ready for dispatch versus tickets missing assignment, lead-tech, schedule, or due-date context, name the next dispatch focus or fix from loaded data, show assigned and lead employee names on list rows when existing user records are available, and filter by dispatch-readiness state on the job-ticket list using existing job-ticket, assignment, and user APIs.
- Manager/Admin job ticket detail readiness cues now summarize assignment ownership, lead tech, customer, service location, equipment or no-equipment context, schedule, and due date using existing job-ticket and assignment data, name the next dispatch fix, and show employee names when loaded.
...
- Employee assigned-job list rows now show due-date and next field-context fix cues from existing assigned-job list data, while Employee job ticket detail field-context cues summarize schedule, due date, customer, service location, equipment or no-equipment context, and job instruction context, name the next field-context fix, and show pre-work guidance when field context still needs manager review using existing assigned-ticket detail data.
...
- Employee mobile workflow for assigned jobs, GPS time tracking, work notes, part usage, files/photos, assigned-job list field-context review, assigned-work field context review, next field-context fix guidance, and pre-work context guidance.
- Manager/Admin job-ticket workflow, assignment management, archive/status UX, reporting hub, inline status/archive review feedback, closeout/invoice-readiness review, dashboard dispatch-readiness summary, dispatch-readiness list cues and filters, detail dispatch-readiness checklist and next-fix cues, edit-side dispatch-readiness cues, and time-review approval workspace.
...
- Manager/Admin job-ticket list rows now show assigned and lead employee names when existing user records are available, with assignment-id fallback when names cannot be loaded;
...
- the Employee assigned-job list now shows due-date and next field-context fix cues from existing list data, while the Employee job-ticket detail page summarizes field context for scheduled start, due date, customer, service location, equipment or no-equipment context, and job instructions, names the first next field-context fix, then surfaces pre-work guidance when manager review is still needed, using existing assigned-ticket detail data.
