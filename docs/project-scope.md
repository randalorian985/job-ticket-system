...
- Manager/Admin Dispatch at `/manage/dispatch` is the shared schedule for active job tickets. It provides Unscheduled Tickets, Today, Tomorrow, and Next 7 Days views; one Schedule & Assign panel; and guarded En Route, On Site, Start Work, Complete Work, and Open Ticket actions backed by existing job-ticket update, assignment, status, and work-entry APIs. Completed-work review stays in the ticket workspace, and billing-ready review stays in Reports.
...
Phase 3B validation hardening keeps whitespace-only required customer, service-location, equipment, vendor, part-category, and part values from being submitted from the Manager/Admin forms. This remains frontend validation and test coverage within the existing master-data workflow; it does not change API contracts, DTO shapes, schema, migrations, authorization, archive behavior, purchasing, inventory, recommendations, AI/scoring, automatic compatibility, or automatic approval.

Phase 3B archive workflow hardening requires Manager/Admin confirmation before customer, service-location, equipment, part, vendor, or part-category archive/unarchive actions are sent to the existing APIs. This remains frontend confirmation and test coverage within the existing soft-delete/archive workflow; it does not change API contracts, DTO shapes, schema, migrations, authorization, purchasing, inventory, recommendations, AI/scoring, automatic compatibility, or automatic approval.
...
Phase 3B equipment year validation hardening keeps Manager/Admin equipment create/edit forms from submitting fractional years or years outside 1900 through 2100, matching the existing job-ticket quick-add equipment form boundary. This remains frontend validation within the existing master-data workflow; it does not change API contracts, DTO shapes, schema, migrations, authorization, archive behavior, purchasing, receiving, inventory expansion, recommendations, AI/scoring, automatic compatibility, or automatic approval.

Phase 3B equipment customer/location alignment hardening keeps Manager/Admin equipment create/edit forms focused on service locations that belong to the selected primary customer and prevents mismatched customer/service-location equipment submissions. This remains frontend relationship validation within the existing master-data workflow; it does not change API contracts, DTO shapes, schema, migrations, authorization, archive behavior, purchasing, receiving, inventory expansion, recommendations, AI/scoring, automatic compatibility, or automatic approval.

Phase 3B filter-driven form defaults prefill blank Manager/Admin master-data create forms from active service-location customer, equipment customer, part category, and preferred vendor filters while preserving selected records during edit mode. This remains frontend form-flow polish within the existing master-data workflow; it does not change API contracts, DTO shapes, schema, migrations, authorization, archive behavior, purchasing, receiving, inventory expansion, recommendations, AI/scoring, automatic compatibility, or automatic approval.

Phase 3B related-record option hardening keeps archived customers, service locations, part categories, and vendors out of blank Manager/Admin master-data create-form relationship selectors while preserving the currently selected archived relationship during edit mode. This remains frontend form-flow hardening within the existing master-data workflow; it does not change API contracts, DTO shapes, schema, migrations, authorization, archive behavior, purchasing, receiving, inventory expansion, recommendations, AI/scoring, automatic compatibility, or automatic approval.

Phase 3B equipment service-location fallback hardening preserves an equipment record's unchanged customer/location relationship when its current service location is not included in the loaded options, labels that retained relationship as unavailable, and prevents the fallback from being reused for another customer or location. This remains frontend relationship validation within the existing master-data workflow; it does not change API contracts, DTO shapes, schema, migrations, authorization, archive behavior, purchasing, receiving, inventory expansion, recommendations, AI/scoring, automatic compatibility, or automatic approval.
...
- focused Dispatch board for active-ticket scheduling, selecting the customer's crane/equipment being serviced, operator/crew assignment, and day-of coordination;
