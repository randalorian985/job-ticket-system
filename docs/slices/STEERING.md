...
- `docs/codex-model-routing.md` defines task classification, model class, exact model selection, reasoning effort, cost routing, review expectations, and escalation rules. It governs execution method and may not change product scope, status, sequence, dependencies, or acceptance criteria.
- `docs/slices/000-slice-planning.md` defines the active sequence and status of every slice.
- This file defines sizing, ownership, and execution rules inherited by every parent and child under `docs/slices/`.
...
- A parent must never be sent to Codex as one implementation or planning run.
- A child must deliver one coherent business capability, migration stage, integration outcome, or planning artifact family.
...
- Preserve IDs, tenant boundaries, historical records, permissions, and working workflows unless the child contains an approved migration stage.
- Follow `AGENTS.md` for repository-wide route, authorization, domain, and deferred-scope boundaries.

## Ownership boundaries
- Core master-data children own canonical records, lifecycle, validation, search, and direct management.
...
- Consistency children may standardize proven implementations but must not rebuild the owned capability.
...
- Record what already worked, what changed, validation performed, known risks, deferred dependencies, selected model class, exact model identifier, cost rationale, and elevation history.
...
3. State the selected model class, exact model identifier or picker label, reasoning effort, cost rationale, and authorized elevation triggers.
...
- `docs/codex-model-routing.md` controls task classification, model selection, reasoning effort, cost routing, and execution method only.
