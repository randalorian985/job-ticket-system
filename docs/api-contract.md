...
- Manager/Admin job-ticket edit dispatch-readiness cues are client-side and use the existing edit DTO fields for customer, service location, equipment, scheduled start, due date, and job instructions. This does not add endpoints or change request/response DTOs.
...
- Job Ticket Dispatch & Assignment Readiness Polish is the selected implementation lane. Current dispatch-readiness polish uses existing job-ticket and assignment APIs plus client-side edit review cues over existing job-ticket fields; any further API changes must be narrow, DTO-based, job-ticket-first, and documented here.
