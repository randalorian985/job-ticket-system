...
- Job Ticket Dispatch & Assignment Readiness Polish is the selected implementation lane. Current dispatch-readiness polish uses existing job-ticket and assignment APIs plus client-side Manager/Admin list/detail/edit and Employee list/detail review cues over existing job-ticket fields; any further API changes must be narrow, DTO-based, job-ticket-first, and documented here.
