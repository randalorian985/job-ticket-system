...
- Manager/Admin create/edit UI now provides a dashboard shortcut to `/manage/job-tickets/new` and focused quick-add inside the job-ticket form. Job type quick-add remains a client-side reference-value convenience over the existing nullable `jobType` field. Service-location quick-add calls the existing `POST /api/service-locations` endpoint, then selects the returned service location into the existing job-ticket DTO. Equipment quick-add calls the existing `POST /api/equipment` endpoint with the selected `customerId` and `serviceLocationId`, then selects the returned equipment ID into the existing job-ticket DTO. Before equipment quick-add save, the UI warns about possible duplicate equipment when the entered name, equipment number, unit number, or serial number exactly matches loaded equipment in the selected customer/service-location context. This warning is client-side and non-blocking. This adds no job-ticket endpoints, request fields, response fields, schema changes, migrations, purchasing behavior, inventory expansion, or accounting behavior.
...
- `POST /api/job-tickets/{jobTicketId}/parts/quick-add` lets an assigned employee or Manager/Admin add a ticket part by required `partNumber`. If the part number matches existing master data, the saved job-ticket part remains linked to that part and may adjust inventory. If no match exists, the saved job-ticket part is unlisted, stores ticket-level part number/name, cost snapshot, billable price snapshot, quantity, notes, and does not create a master part or inventory transaction.
- Quick-add request fields: `partNumber`, `partName`, `quantity`, `unitCost`, `salePrice`, `notes`, `isBillable`, `addedByEmployeeId`, `addedAtUtc`, `requestOfficeOrder`, `officeOrderNotes`, `adjustInventory`, `allowManagerOverride`, plus the existing optional compatibility capture fields.
- Job-ticket part responses include nullable `partId`, `partNumber`, `partName`, `isUnlistedPart`, `officeOrderRequested`, `officeOrderRequestedAtUtc`, and `officeOrderNotes`. Manager/Admin responses may include cost and billable price snapshots; employee-safe responses continue to hide price snapshots.
...
- Manager/Admin job-ticket creation quick-add remains within the current API surface by using existing master-data create endpoints only where a ticket form needs an immediate service-location or equipment value.
