...
public sealed class VendorsController(IVendorsService service) : MasterDataControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<VendorDto>> CreateAsync([FromBody] CreateVendorDto request, CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    [HttpPut("{id:guid}")]
    public async Task<ActionResult<VendorDto>> UpdateAsync(Guid id, [FromBody] UpdateVendorDto request, CancellationToken cancellationToken = default)
    {
        try
        {
