...
public sealed class ReportingServiceTests
{
    ...
    private static async Task<SeedRefs> SeedAsync(ApplicationDbContext context)
    {
        ...
        var approvedPart = new JobTicketPart { JobTicketId = mainJob.Id, PartId = part.Id, PartNumberSnapshot = part.PartNumber, PartNameSnapshot = part.Name, Quantity = 2m, UnitCostSnapshot = 10m, SalePriceSnapshot = 25m, ApprovalStatus = JobPartApprovalStatus.Approved, AddedAtUtc = DateTime.UtcNow, Status = PartTransactionStatus.Used };
        context.JobTicketParts.AddRange(
            approvedPart,
            new JobTicketPart { JobTicketId = mainJob.Id, PartId = part.Id, PartNumberSnapshot = part.PartNumber, PartNameSnapshot = part.Name, Quantity = 1m, UnitCostSnapshot = 9m, SalePriceSnapshot = 20m, ApprovalStatus = JobPartApprovalStatus.Pending, AddedAtUtc = DateTime.UtcNow, Status = PartTransactionStatus.Used },
            new JobTicketPart { JobTicketId = mainJob.Id, PartId = part.Id, PartNumberSnapshot = part.PartNumber, PartNameSnapshot = part.Name, Quantity = 1m, UnitCostSnapshot = 9m, SalePriceSnapshot = 20m, ApprovalStatus = JobPartApprovalStatus.Rejected, AddedAtUtc = DateTime.UtcNow, Status = PartTransactionStatus.Used },
            new JobTicketPart { JobTicketId = mainJob.Id, PartId = part.Id, PartNumberSnapshot = part.PartNumber, PartNameSnapshot = part.Name, Quantity = 1m, UnitCostSnapshot = 9m, SalePriceSnapshot = 20m, ApprovalStatus = JobPartApprovalStatus.Approved, AddedAtUtc = DateTime.UtcNow, Status = PartTransactionStatus.Used, IsDeleted = true, DeletedAtUtc = DateTime.UtcNow });
