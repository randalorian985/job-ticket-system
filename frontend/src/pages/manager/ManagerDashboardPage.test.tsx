...
describe('ManagerDashboardPage', () => {
  ...
  it('renders the refreshed operations board and assignment summary from loaded manager job data', async () => {
    ...
    expect(within(nextAction).getByRole('link', { name: 'JT-1' })).toHaveAttribute(
      'href',
      '/manage/job-tickets/j1?returnTo=%2Fmanage'
    )
    expect(within(nextAction).getByText('needs at least one assigned employee.')).toBeInTheDocument()
    expect(within(nextAction).getByRole('link', { name: 'Schedule' })).toHaveAttribute(
      'href',
      '/manage/job-tickets/j1?returnTo=%2Fmanage'
    )
