...
export function EquipmentPage() {
  ...
  const availableServiceLocations = useMemo(() => locations.filter((location) => !draft.customerId || location.customerId === draft.customerId), [locations, draft.customerId])
  const save = async (event: FormEvent) => {
    event.preventDefault()
    ...
    if (!hasMatchingEquipmentServiceLocation(draft.customerId, draft.serviceLocationId, locations)) return setError('Equipment service location must belong to the selected customer.')
    if (!hasValidEquipmentYear(draft.year)) return setError('Equipment year must be a whole year from 1900 through 2100.')
    try {
      setError(null)
...
export function PartsPage() {
  ...
  const save = async (event: FormEvent) => {
    ...
    if (!hasNonNegativeNumbers(draft.unitCost, draft.unitPrice, draft.quantityOnHand, draft.reorderThreshold)) return setError('Part cost, price, quantity on hand, and reorder threshold must be zero or greater.')
    try {
      setError(null)
      if (editId) await masterDataApi.updatePart(editId, draft)
      else await masterDataApi.createPart(draft)
      setDraft(emptyPartDraft); setEditId(null); await load()
    } catch (requestError) {
