...
Ticket-detail links may include `returnTo` so breadcrumbs return the user to the exact queue; the visible label is derived from the validated internal path rather than URL-provided display text. Detail ticket tabs use `tab` with one of `overview`, `dispatch`, `time`, `parts`, `files`, `closeout`, or `activity`; the `dispatch` tab value is the legacy internal value for the user-facing Technicians tab. Next actions, direct ticket-tab selection, and action-rail shortcuts set `view=workflow` when the selected section should be the primary screen. `view=workflow` hides the overview rail, keeps the selected panel directly under the ticket tabs, and is especially important for mobile action visibility.
