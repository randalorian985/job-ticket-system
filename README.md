...
- Manager/Admin job ticket list, detail, and edit readiness review surfaces identify the next dispatch fix for missing assignment, lead tech, schedule, due date, or job-instruction context using existing loaded data, with list and detail ownership labels resolving employee names through assignment responses when available.
- Manager/Admin job ticket detail and edit readiness review for assignment, lead tech, customer, service location, equipment or no-equipment context, schedule, due date, and job instructions before dispatch handoff, including active-status-aware detail dispatch readiness and named assignment ownership on detail from assignment responses with employee-record and ID fallback.
...
- Employee mobile job workflow with GPS time tracking, work notes, part usage, files/photos, assigned-job list field-context review, assigned-work field context review, next field-context fix guidance, and context guidance before starting work.
