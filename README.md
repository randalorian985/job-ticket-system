...
- Manager/Admin dashboard now has a direct Create Job Ticket CTA, and the job-ticket create/edit form supports focused quick-add for job type values, service locations, and equipment while selecting the newly added value back into the ticket form. Service-location and equipment quick-add use existing master-data APIs, preserve the selected customer/service-location context, warn about possible duplicate equipment from loaded equipment records, and do not add purchasing, inventory, accounting, or broad master-data workflow scope.
