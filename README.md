...
- Manager/Admin dashboard now has a direct Create Job Ticket CTA, and the job-ticket create/edit form supports focused quick-add for job type values, service locations, and equipment while selecting the newly added value back into the ticket form. Service-location and equipment quick-add use existing master-data APIs, preserve the selected customer/service-location context, and equipment quick-add warns when the draft matches loaded equipment for the selected customer/location before creating another record. This does not add purchasing, inventory, accounting, or broad master-data workflow scope.
...
- Employee job detail now supports ticket-level quick-add parts by required part number, including catalog matching when the part exists, one-off unlisted part capture when it does not, cost and billable price snapshots, and optional office-order request context.
...
- Manager/Admin job ticket list, detail, and edit readiness review surfaces identify the next dispatch fix for missing assignment, lead tech, schedule, due date, active dispatch status, or job-instruction context using existing loaded data, with list and detail ownership labels resolving employee names through assignment responses when available.
...
- Manager/Admin dashboard create-ticket shortcut plus job-ticket create/edit quick-add for job type reference values, service locations, and equipment, using existing service-location and equipment master-data APIs, preserving the selected customer/service-location relationship, and warning on possible duplicate equipment during quick-add.
...
- Employee mobile job workflow with GPS time tracking, work notes, part usage, files/photos, active-status-aware assigned-job list/detail field-context review, next field-context fix guidance, and context guidance before starting work.
- Employee parts-on-ticket quick-add for catalog-matched and one-off unlisted parts without expanding the master parts, purchasing, or inventory domains.
