...
- Manager/Admin dashboard and job ticket list dispatch-readiness cues now summarize active tickets that are ready for dispatch versus tickets missing assignment, lead-tech, schedule, or due-date context, identify the next dispatch focus or fix from loaded data, show assigned and lead employee names on list rows when existing user records are available, and filter by those dispatch-readiness states on the job-ticket list using existing job-ticket, assignment, and user APIs.
...
- Manager/Admin job ticket list, detail, and edit readiness review surfaces identify the next dispatch fix for missing assignment, lead tech, schedule, due date, or job-instruction context using existing loaded data, with list/detail ownership labels resolving employee names when available.
